...
def expand_query(query_tokens_ix, query_tokens_term, vocab_to_ix):
    expanded_query = copy.deepcopy(query_tokens_ix)
    words_compressed = np.load('u.npy')
    docs_compressed = np.load('v_trans.npy')
    words_compressed = normalize(words_compressed, axis = 1)
    word_to_ix = pickle.load(open('allthenews_vocab_ix.p', 'rb'))
    reu_word_to_ix = json.load(open("vocab_to_ix.json"))
    ix_to_word = {i:t for t,i in word_to_index.iteritems()}
    for tok in query_tokens_term:
        syns = closest_words(tok, words_compressed, word_to_ix, ix_to_word)
        for w in syns:
            reu_ix = reu_word_to_ix.get(w, -1)
            if reu_ix != -1:
                expanded_query[reu_word_to_ix[w]] = .2
    gc.collect()
